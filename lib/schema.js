  ...
  function loadSchema(schema, namespace) {
    ...
    function registerSchema() {
      function registerSymbol(name) {
        ...
        if (!~name.indexOf('.')) {
          name = schema.namespace || namespace + name;
        }
        if (name in symbols) {
    ...
    switch (typeName) {
      ...
      case 'record':
        return registerSchema() && schema.fields.every(function (field) {
  ...
  function validateSchema(datum, schema) {
    ...
    switch (typeName) {
      ...
      case 'map':
        ...
        return true;
      case 'enum':
        return schema.symbols.some(function(symbol) {
          return symbol === datum;
        });
      case 'record':
